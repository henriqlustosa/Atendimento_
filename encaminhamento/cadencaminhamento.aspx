...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <!-- PAINEL QUE RECEBE O ENTER COMO "GRAVAR" -->
    <asp:Panel ID="pnlDefaultEnter" runat="server" DefaultButton="btnGravar">
    ...
    <script>
        ...
        function anyExamSelected() {
            ...
            $('[data-exam]').each(function () {
                ...
                if (v && ($.isArray(v) ? v.length > 0 : $.trim(v).length > 0)) {
                    ok = true;
                    return false;
                }
            });
        ...
        function validateAlgumExame(sender, args) {
            args.IsValid = anyExamSelected();
        }
                ...
                if (typeof (Page_Validators) !== 'undefined') {
                    for (var i = 0; i < Page_Validators.length; i++) {
                        ...
                        if (v && v.validationGroup === 'Salvar' && v.isvalid === false) {
                            invalid = document.getElementById(v.controltovalidate);
                            break;
                        }
                    }
                }
        ...
        // ---- Popup de erro para RH / prontuário ----
        function mostrarErroRh(mensagem) {
            alert(mensagem);
            var $rh = $('#<%= txbProntuario.ClientID %>');
            var $nome = $('#<%= txbNomePaciente.ClientID %>');
            $rh.val('');
            $nome.val('');     // LIMPA O NOME DO PACIENTE
            setTimeout(function () {
                $rh.focus();
            }, 10);
        }

        // ---- Abrir accordion Pedido e focar na Data ----
        function abrirPedidoEFocarData() {
            var card = document.getElementById("cPedido");
            if (card) {
                var acc = bootstrap.Collapse.getOrCreateInstance(card, { toggle: false });
                acc.show();

                setTimeout(function () {
                    $('#<%= txbDtPedido.ClientID %>').focus();
                }, 250);
            }
        }

        // ---- ENTER no RH/Prontuário (delegação) ----
        // Funciona mesmo depois do UpdatePanel recriar o input
        $(document).on('keydown', '#<%= txbProntuario.ClientID %>', function (e) {
            if (e.key === 'Enter' || e.which === 13) {
                ...
                if (valor === '') {
                    // RH vazio
                    mostrarErroRh('Informe o prontuário do paciente.');
                    return;
                }

                // Dispara a pesquisa (AsyncPostBackTrigger do UpdatePanel)
                $('#<%= SearchButton.ClientID %>').click();
            }
        });

        // ---- Após QUALQUER endRequest (no seu caso, só do SearchButton) ----
        // Se Nome preenchido → paciente encontrado → abre Pedido e foca Data
        // Se Nome vazio     → não achou → popup e volta foco pro RH
        if (typeof (Sys) !== 'undefined' &&
            Sys.WebForms &&
            Sys.WebForms.PageRequestManager) {

            var prm = Sys.WebForms.PageRequestManager.getInstance();
            if (prm && !prm._cadEncEndReqBound) {
                prm.add_endRequest(function (sender, evArgs) {
                    var nome = $('#<%= txbNomePaciente.ClientID %>').val();
                    if (nome && $.trim(nome) !== '') {
                        abrirPedidoEFocarData();
                    } else {
                        // Só mostra erro se o RH não estiver vazio (pra não interferir em outras coisas futuras)
                        var rhVal = $.trim($('#<%= txbProntuario.ClientID %>').val());
                        if (rhVal !== '') {
                            mostrarErroRh('Prontuário não encontrado. Verifique o número digitado.');
                        }
                    }
                });
                prm._cadEncEndReqBound = true;
            }
        }
        ...
        $(function () {
            ...
            $('.selectpicker').each(function () {
                ...
                dedupeOptionsByText($s);
                toggleSearchBox($s);
                ensureUniqueSelection($s);
            ...
            function toggleSearchBox($s) {
                $s.parent('.bootstrap-select')
                    .toggleClass('no-search', $s.find('option').length < 8);
            }
